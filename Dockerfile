...
RUN GPG_KEYS=B0F4253373F8F6F510D42178520A9993A1C052F8 \
	...
	&& wget -O "/usr/src/pcre-${PCRE_VERSION}.tar.gz" "http://linux.stanford.edu/pub/exim/pcre/pcre-${PCRE_VERSION}.tar.gz" \
	...
	&& sed -i "s|\"Server:\ \"\ NGINX_VER\ CRLF;|\"Server:\ $SERVER_NAME\"\ CRLF;|g" /usr/src/nginx-$NGINX_VERSION/src/http/ngx_http_header_filter_module.c \
	#For security reasons change error pages
	&& sed -i "s|\ bgcolor=\\\\\"white\\\\\"||g" /usr/src/nginx-$NGINX_VERSION/src/http/ngx_http_special_response.c \
	&& sed -i "s|<hr><center>nginx</center>||g" /usr/src/nginx-$NGINX_VERSION/src/http/ngx_http_special_response.c \
	&& sed -i "s|<!--\ a\ padding\ to\ disable\ MSIE\ and\ Chrome\ friendly\ error\ page\ -->||g" /usr/src/nginx-$NGINX_VERSION/src/http/ngx_http_special_response.c \
